---
layout: layouts/home.njk
title: PRAISe Lab
description: The Perception, Robotics, AI and Sensing (PRAISe) Lab at CU Boulder builds minimal perception and autonomy for resource-constrained robots and tiny drones.
permalink: /index.html

hero_slides:
  # - title: Chahat Deep Singh joins CU Boulder as an Assistant Professor in Robotics
  #   text_url: https://colorado.edu
  #   image_url: https://colorado.edu
  #   image: /img/banners/boulder-campus.png
  #   alt: University of Colorado Boulder campus and Flatirons
  #   fit: cover


  - title: Submit to Insect-scale Autonomy IROS workshop | Deadline - Sep 1, 2026
    text_url: https://insect-autonomy.github.io/
    image_url: https://insect-autonomy.github.io/
    image: https://insect-autonomy.github.io/insect-autonomy-banner.jpg
    alt: Insect-scale Autonomy Workshop
    fit: cover

  - title: Ajna is published and featured on the cover of Science Robotics
    text_url: https://prg.cs.umd.edu/Ajna
    image_url: https://prg.cs.umd.edu/Ajna
    image: /img/slider/AjnaCover.png
    alt: Ajna uncertainty estimation research
    fit: cover

  - title: A new camera, named AMI-EV, was invented and published in Science Robotics
    text_url: https://prg.cs.umd.edu/AMI-EV
    image_url: https://prg.cs.umd.edu/AMI-EV
    image: /img/slider/ami_ev.png
    alt: AMI-EV event-camera research
    fit: contain

  - title: CodedVO is published in the IEEE Robotics and Automation Letters journal
    text_url: https://prg.cs.umd.edu/CodedVO
    image_url: https://prg.cs.umd.edu/CodedVO
    image: /img/slider/CodedVO.jpg
    alt: Coded Visual Odometry research
    fit: contain

  - title: RoboBeeHive featured in a BBC Earth documentary
    text_url: https://www.youtube.com/watch?v=ed3jg-xQfPw
    image_url: https://prg.cs.umd.edu/CodedVO
    image: /img/slider/RoboBeeHive.png
    alt: Tiny drone pollination and RoboBeeHive research
    fit: contain
---

<!-- Latest 8 News is auto fetched from news.md -->


Welcome to **P**erception, **R**obotics, **AI** and **Se**nsing
(**PRAISe**) Lab, led by Prof.
[Chahat Deep Singh](https://chahatdeep.github.io/).

We work at the intersection of perception, robotics, artificial intelligence,
and computational imaging to push the boundaries of robot autonomy. Our
**Minimal Perception** framework focuses on developing small, intelligent, and
efficient robot-navigation systems through active and bio-inspired perception,
novel sensing, and onboard computation.

Prospective students can review the lab's current opportunities on the
[Open Positions](/openings) page.