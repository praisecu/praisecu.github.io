...
FileList = [f for f in listdir(FolderName) if isfile(join(FolderName, f)) and '.pdf' in f]
...
def main():
	...
	#    SavePath = [StudentName+'.jpg']
		...
		midpoint = int(ImgStacked.shape[1] / 2)
		ImgTop = ImgStacked[:, :midpoint, :]
		ImgBottom = ImgStacked[:, midpoint:, :]

		ImgStacked = np.vstack((ImgTop, ImgBottom))

		cv2.imwrite(FolderName+FileName+'.png', ImgStacked)
